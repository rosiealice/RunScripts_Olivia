...
if [[ $# -ne 1 ]]; then
  echo "Usage: $0 <CSTARV|GRAZ|MORT|RAD|VCM|GRR|MDSL>" >&2
  exit 1
fi
...
BEGIN { inserted = 0 }
{
  # Handle current default-script pattern: name="${NAME_OVERRIDE:-default}"
  if ($0 ~ /^[[:space:]]*name=\"\$\{NAME_OVERRIDE:-[^}]+\}\"/) {
    print "name=\"${NAME_OVERRIDE:-" tag "}\" #name of this setup, used for naming the case and directories."
    next
  }

  # Backward-compatible fallback for older pattern: name='default'
  if ($0 ~ /^[[:space:]]*name='\''[^'\'']*'\''/) {
    print "name='\''" tag "'\'' #name of this setup, used for naming the case and directories."
    next
  }

  # Remove any existing fates_paramfile assignment before reinserting one.
