...
name="${NAME_OVERRIDE:-default}" #name of this setup, used for naming the case and directories. 

dosetup1=${DOSETUP1_OVERRIDE:-1} #do first part of setup
dosetup2=${DOSETUP2_OVERRIDE:-1} #do second part of setup (after first manual modifications)
dosetup3=${DOSETUP3_OVERRIDE:-1} #do second part of setup (after namelist manual modifications)
dosubmit=${DOSUBMIT_OVERRIDE:-1} #do the submission stage
forcenewcase=${FORCENEWCASE_OVERRIDE:-0} #scurb all the old cases and start again
...
rundate="${RUNDATE_OVERRIDE:-$(date +"%Y-%m-%d")}"
casename="amazon_ppe.$resolution.$noresmversion.CRUJRA_actual2000_$name.$rundate"
echo "casename: $casename"
compset="2000_DATM%CRUJRA2024_CLM60%FATES-NOCOMP_SICE_SOCN_SROF_SGLC_SWAV_SESP"
...
then
    ...
    then 
        ...
        rm -rf /cluster/projects/nn9560k/$USER/$casename
        rm -r /cluster/work/projects/nn9560k/$USER/noresm/$casename
        rm -r /cluster/work/projects/nn9560k/$USER/archive/$casename
        rm -r $casename
	fi
    fi
    if [[ -d "$scriptsdir$casename" ]] 
...
fluh_timeseries='/cluster/work/projects/nn9560k/inputdata/LU_data_CMIP7/LUH2_states_transitions_management.timeseries_ne16_hist_steadystate_2000_2025-11-06_cdf5.nc'
